...
export function registerTimeTrackingTools(
  ...
  server.tool(
    ...
    {
      ids: z.array(z.string()).optional().describe("Filter by specific time tracking entry IDs"),
      user_id: z.string().optional().describe("Filter by the user who logged the time"),
      subject_type: z
        .enum(TIME_TRACKING_SUBJECT_TYPES)
        .optional()
        .describe("Type of the entity time was logged against (with subject_id)"),
      subject_id: z.string().optional().describe("ID of the entity time was logged against (with subject_type)"),
      started_after: z.string().optional().describe("ISO 8601 datetime — only entries started at/after this"),
      started_before: z.string().optional().describe("ISO 8601 datetime — only entries started at/before this"),
      ended_after: z.string().optional().describe("ISO 8601 datetime — only entries ended at/after this"),
      ended_before: z.string().optional().describe("ISO 8601 datetime — only entries ended at/before this"),
    ...
    async (params) => {
      ...
      if (params.started_after) filter.started_after = params.started_after;
      if (params.started_before) filter.started_before = params.started_before;
      if (params.ended_after) filter.ended_after = params.ended_after;
      if (params.ended_before) filter.ended_before = params.ended_before;
  ...
  server.tool(
    ...
    { id: z.string().describe("The time tracking entry ID") },
    async (params) => {
  ...
  server.tool(
    ...
    "Log time worked (e.g. '1 hour of effort') against a subject such as a task/todo, ticket, milestone, event, company, or contact. " +
      ...
      "Note: Teamleader splits an entry that spans midnight into multiple entries automatically.",
    {
      ...
      subject_id: z.string().describe("ID of the entity to log time against"),
      started_at: z
        .string()
      ...
      work_type_id: z.string().optional().describe("Work type ID (see teamleader_list_work_types)"),
      invoiceable: z.boolean().optional().describe("Whether this time is billable to the customer"),
      user_id: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    {
      id: z.string().describe("The time tracking entry ID to update"),
      duration_minutes: z.number().optional().describe("New duration in minutes"),
      duration_seconds: z.number().optional().describe("New duration in seconds"),
      ...
      subject_id: z.string().optional().describe("New subject ID (with subject_type)"),
      started_at: z.string().optional().describe("New start datetime, ISO 8601"),
      invoiceable: z.boolean().optional().describe("Whether this time is billable"),
    },
    async (params) => {
