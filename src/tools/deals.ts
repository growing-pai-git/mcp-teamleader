...
export function registerDealTools(
  ...
  server.tool(
    ...
    "Update an existing deal in Teamleader Focus. Supports relinking the customer (lead), editing the description (summary), value, closing date, probability, responsible user, source and department. Note: to change the deal's phase use teamleader_move_deal, and to mark it won/lost use teamleader_win_deal / teamleader_lose_deal.",
    {
      ...
      responsible_user_id: z
        ...
        .optional()
        .describe("Responsible user ID"),
      source_id: z.string().optional().describe("Deal source ID"),
      department_id: z.string().optional().describe("Department ID"),
    },
    async (params) => {
  ...
  server.tool(
    ...
    "Move a deal to a different phase in its pipeline (e.g. advance it to a later stage). Use teamleader_list_deal_phases or the deal's pipeline to find the target phase_id.",
    {
      id: z.string().describe("The deal ID"),
      phase_id: z.string().describe("The target deal phase ID to move the deal into"),
    },
    async (params) => {
      await client.request({
        endpoint: "deals.move",
  ...
  server.tool(
    "teamleader_win_deal",
    "Mark a deal as won. This formally flips the deal's status to 'won' in Teamleader Focus.",
    {
  ...
  server.tool(
    "teamleader_lose_deal",
    "Mark a deal as lost, optionally with a lost reason and extra info.",
    {
  ...
  server.tool(
    ...
    "Delete a deal from Teamleader Focus. This is irreversible.",
    {
