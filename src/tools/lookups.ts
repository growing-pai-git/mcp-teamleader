...
const PRODUCT_SAFE_FIELDS = ["id", "name", "description", "code", "unit", "tax"] as const;

function redactProductForNonAdmin(product: Record<string, unknown>): Record<string, unknown> {
...
function registerSimpleList(
  ...
  opts: {
    ...
    /** Optional per-item redaction applied to each element of `data` before returning. */
    redactItem?: (item: Record<string, unknown>) => Record<string, unknown>;
  }
): void {
  server.tool(
    opts.name,
    opts.description,
    {
      ...
      ...(opts.supportsTerm
        ? { term: z.string().optional().describe("Search term filter") }
        : {}),
    ...
    async (params: Record<string, unknown>) => {
      ...
      if (params.page || params.page_size) {
        body.page = {
          number: (params.page as number) ?? 1,
          size: (params.page_size as number) ?? 20,
        };
      }
      ...
      const result = (await client.request({ endpoint: opts.endpoint, body })) as {
...
export function registerLookupTools(
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if (params.context) body.filter = { context: params.context };
      const result = await client.request({
