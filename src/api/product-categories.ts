...
export const DEFAULT_LEDGER_ACCOUNT_NUMBER = Number(
  process.env.TEAMLEADER_DEFAULT_LEDGER_ACCOUNT ?? 700000
);
 ...
 */
export async function resolveDefaultProductCategoryId(
  ...
  const body: Record<string, unknown> = { page: { number: 1, size: 100 } };
  if (departmentId) body.filter = { department_id: departmentId };

  const result = await client.request<{ data?: ProductCategory[] }>({
  ...
  const match = categories.find((category) =>
    category.ledgers?.some(
      (ledger) =>
        Number(ledger.ledger_account_number) === DEFAULT_LEDGER_ACCOUNT_NUMBER &&
        (!departmentId ||
  ...
  if (!match) {
    throw new Error(
      `No product category ("Rekening") found with ledger account ` +
